...
import (
	...
	api "github.com/seanchann/apimaster/pkg/apis/coreres"
	v1 "github.com/seanchann/apimaster/pkg/apis/coreres/v1"
	"github.com/seanchann/apimaster/pkg/apis/coreres/validation"
...
func (namespaceStrategy) PrepareForCreate(ctx context.Context, obj runtime.Object) {
	...
	hasCoreResource := false
	for i := range namespace.Spec.Finalizers {
		if namespace.Spec.Finalizers[i] == api.FinalizerCoreResource {
			hasCoreResource = true
			break
		}
	}
	if !hasCoreResource {
		if len(namespace.Spec.Finalizers) == 0 {
			namespace.Spec.Finalizers = []api.FinalizerName{api.FinalizerCoreResource}
		} else {
			namespace.Spec.Finalizers = append(namespace.Spec.Finalizers, api.FinalizerCoreResource)
		}
	}
}
